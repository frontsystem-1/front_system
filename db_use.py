...
class SwitchDB(object):
	...
	def notification(day,time,name,nb):
	    ...
	    data = {
	    "token":'xoxp-5191608585938-5191476628739-5514736172821-5b636ec84622ba8ce68a27ab62784a04',
	...
	def mb(self,judgment):
	    ...
	    try:
		    ...
		    #time.sleep(2)
		    #↓の読み込みが早すぎるとcard_recordが読み込めずにnew_recordがerrorになる
...
while True:
    try:
	    ...
	    switch_db.mb('no')
	    #new_record = switch_db.select_card_record(switch_db.day,cr) #日付とカードのデータに一致した記録
	    #if new_record is not None: #日付とカードのデータに一致した記録がある場合
		    #cursor.execute(f"UPDATE door_record SET exit_time = '%s' ORDER BY exit_time DESC LIMIT 1" % (str(new_record[1])[11:19]))
		    #connection.commit()
